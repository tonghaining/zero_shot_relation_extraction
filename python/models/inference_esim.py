...
class MyModel(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings, emb_train, description_num):
        ...
        m_a = tf.concat([premise_bi_mean, premise_attns_mean, prem_diff_mean, prem_mul_mean], 2)
        m_b = tf.concat([hypothesis_bi_mean, hypothesis_attns_mean, hyp_diff_mean, hyp_mul_mean], 2)

        ### Inference Composition ###

        v2_outs, c4 = blocks.biLSTM(m_b, dim=self.dim, seq_len=hyp_seq_lengths_mean, name='v2')
        ...
        with tf.variable_scope("conditional_inference_composition-v1") as v1_scope:
            v1_outs, c3 = blocks.reader(m_a, prem_seq_lengths_mean, self.dim, c4, scope=v1_scope)
